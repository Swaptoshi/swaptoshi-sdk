...
export class FeeConversionModule extends Modules.BaseModule {
	...
	public addDependencies(dependencies: FeeConversionModuleDependencies) {
		if ((dependencies.feeMethod as { getConfig? }).getConfig === undefined) throw new Error("feeMethod doesn't have getConfig method, patch needed");
	...
	public async beforeCommandExecute(_context: StateMachine.TransactionExecuteContext): Promise<void> {
		...
		if ((this._feeMethod! as { getConfig? }).getConfig === undefined) throw new Error("feeMethod doesn't have getConfig method, patch needed");

		const config = (this._feeMethod! as { getConfig? }).getConfig();
	...
	public _getMinFee(blockHeight: number, transactionByteLength: number): bigint {
		...
		const feeConfig = (this._feeMethod! as { getConfig? }).getConfig();
