...
import { Uint, Uint128, Uint256 } from '@dist/stores/library/int';

import * as tickMath from '@dist/stores/library/core/tick_math';
...
import { DexModuleConfig, TokenMethod } from '@dist/types';
						...
						describe(description, () => {
							...
							it('sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity', async () => {
								...
								const [amount0CollectAndBurn, amount1CollectAndBurn] = await pool
									.createEmulator()
									.collect(arbitrageur, tickLower.toString(), tickUpper.toString(), Uint128.MAX, Uint128.MAX);
