...
import { NonfungiblePositionManager, SwapRouter } from '@dist/stores/factory';
...
import { MutableSwapContext, TokenMethod } from '@dist/types';
			...
			describe('multi-pool', () => {
				...
				it('events', async () => {
					...
					expect(mock_token_transfer).toHaveBeenCalledWith(
						(await getPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM)).address.toString('hex'),
					...
					expect(mock_token_transfer).toHaveBeenCalledWith(
						router.address.toString('hex'),
						(await getPool(tokens[1].address, tokens[2].address, FeeAmount.MEDIUM)).address.toString('hex'),
						'3',
					);
