...
import * as nftDescriptor from '@dist/stores/library/periphery/nft_descriptor';
import * as NFTSVG from '@dist/stores/library/periphery/nft_svg';
...
describe('NFTDescriptor', () => {
	...
	const nftDescriptorFixture: Fixture<{ tokens: [Tokens, Tokens, Tokens, Tokens] }> = async (_sender: Buffer) => {
		...
		const { token0, token1, token2, token3, token0Decimal, token0Symbol, token1Decimal, token1Symbol, token2Decimal, token2Symbol, token3Decimal, token3Symbol } = await poolFixture(
			context,
			module,
		);
	...
	describe('#svgImage', () => {
		...
		it('matches the current snapshot', async () => {
			...
			fs.writeFileSync('./test/unit/stores/library/periphery/__snapshots__/nft_descriptor.svg', svg);
		});
