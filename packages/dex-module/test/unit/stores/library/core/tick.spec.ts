...
import * as tickTest from '@dist/stores/library/core/tick';
import { TickInfoStore } from '@dist/stores/tick_info';
import { TickInfo } from '@dist/types';
import { Uint256 } from '@dist/stores/library/int';
...
describe('Tick', () => {
	...
	describe('#clear', () => {
		it('deletes all the data in the tick', async () => {
			...
			const { feeGrowthOutside0X128, feeGrowthOutside1X128, secondsOutside, secondsPerLiquidityOutsideX128, liquidityGross, tickCumulativeOutside, liquidityNet, initialized } = await getTick(
				'2',
			);
			expect(feeGrowthOutside0X128).toBe('0');
			expect(feeGrowthOutside1X128).toBe('0');
